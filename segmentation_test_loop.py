...
img1_t = cv2.resize(img1, (2688/4, 1520/4))
img1 = transform(img1_t).unsqueeze(0).to(device)

original_img2 = cv2.imread('street3.jpg')
img2 = cv2.cvtColor(original_img2, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
img2_t = cv2.resize(img2, (2688/4, 1520/4))
img2 = transform(img2_t).unsqueeze(0).to(device)
...
# masked_image = cv2.bitwise_and(, original_img, mask=road_mask)
